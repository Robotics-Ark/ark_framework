...
def encode_sample(space: Space, sample: Any) -> bytes:
    """Encode a Gymnasium space sample into compact binary bytes.
...
def decode_sample(space: Space, payload: BytesLike, *, copy: bool = False) -> Any:
    """Decode bytes produced by encode_sample for a Gymnasium space.
...
def encode_space(space: Space) -> bytes:
    """Encode a Gymnasium space schema into compact binary bytes."""
    writer = _BinaryWriter()
...
def decode_space(payload: BytesLike) -> Space:
    """Decode bytes produced by encode_space into a Gymnasium space."""
    reader = _BinaryReader(payload)
...
def _encode(space: Space, sample: Any, writer: _BinaryWriter) -> None:
    if isinstance(space, Box):
        ...
        raise TypeError(f"Unsupported Gymnasium space type: {type(space).__name__}.")
...
def _decode_space(reader: _BinaryReader) -> Space:
    ...
    if tag == _SPACE_BOX:
        ...
        raise ValueError(f"Unsupported Gymnasium space tag: {tag}.")
