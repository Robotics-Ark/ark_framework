"""MuJoCo physics backend for the Ark simulator.

Requires the ``mujoco`` Python package (``pip install mujoco``).

asset_cfg schema::

    mjcf: path/to/scene.xml           # required: MJCF/XML model file
    robots:
      - name: panda
        joint_groups:
          - name: arm
            joint_names: [joint1, joint2, ...]
            actuator_names: [act1, act2, ...]  # one per joint, maps to data.ctrl
            control_mode: JOINT_POSITION       # or JOINT_TORQUE
        sensors:                               # optional robot-attached sensors
          - name: wrist_ft
            type: ft                           # reads named MuJoCo sensors
            sensor_names: [force_sensor, torque_sensor]  # concatenated
    objects:
      - box                                    # body names whose pose to publish
    sensors:                                   # standalone sensors
      - name: cam0
        type: rgb                              # rgb | depth | rgbd
        camera_name: frontview                 # camera name in MJCF
        height: 480
        width: 640
...
dr_cfg schema::
    ...
    gravity:
      z_range: [-10.5, -8.5]
    bodies:
      - name: link1                            # body name in MJCF
        mass_scale_range: [0.9, 1.1]
    dofs:
      - name: joint1                           # joint name in MJCF
        damping_range: [0.01, 0.5]
    geoms:
      - name: floor
        friction_scale_range: [0.8, 1.2]      # multiplier on all friction coefficients
"""
